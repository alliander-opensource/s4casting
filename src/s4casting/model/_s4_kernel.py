...
def _conj(x):
    """Returns the conjugate of a tensor, handling complex and real types.

    Args:
        x: input tensor of real or complex dtype
    Returns:
        conjugated tensor
    """
...
def _resolve_conj(x):
    """Resolves conjugate pairs in the last dimension of a tensor.

    Args:
        x: input tensor of real or complex dtype

    Returns:
        tensor with conjugate pairs resolved
    """
...
def _broadcast_dims(*tensors):
    """Broadcasts tensors to have the same number of dimensions.

    Args:
        *tensors: input tensors to broadcast

    Returns:
        list of broadcasted tensors
    """
    max_dim = max(len(tensor.shape) for tensor in tensors)
    return [tensor.view((1,) * (max_dim - len(tensor.shape)) + tensor.shape) for tensor in tensors]


def cauchy_keops(v, z, w):
    """Computes the Cauchy kernel using KeOps for efficient GPU-based computation.

    Args:
        v (torch.Tensor): Input tensor representing the numerator of the Cauchy kernel.
            Shape: (..., N), where `N` is the number of elements.
        z (torch.Tensor): Input tensor representing the evaluation points of the kernel.
            Shape: (..., L), where `L` is the number of evaluation points.
        w (torch.Tensor): Input tensor representing the denominator of the Cauchy kernel.
            Shape: (..., N), where `N` is the number of elements.

    Returns:
        torch.Tensor: Resulting tensor after applying the Cauchy kernel.
            Shape: (..., L), where `L` is the number of evaluation points.
    """
...
def log_vandermonde_keops(v, x, L):
    """Computes the Vandermonde matrix using KeOps for efficient GPU-based computation.

    Args:
        v (torch.Tensor): Input tensor representing the coefficients.
        x (torch.Tensor): Input tensor representing the evaluation points.
        L (int): Number of evaluation points.

    Returns:
        torch.Tensor: Resulting tensor after applying the Vandermonde matrix.
    """
    ...
    lvec = torch.arange(L).to(x)
    v, x, lvec = _broadcast_dims(v, x, lvec)
    v = _c2r(v)
    x = _c2r(x)
    lvec = _c2r(lvec)

    r = vandermonde_mult(v, x, lvec, backend="GPU")
    return 2 * _r2c(r).real


def log_vandermonde_transpose_keops(u, v, x, L):
    """Computes the transpose of the Vandermonde matrix using KeOps for efficient GPU-based computation.

    Args:
        u (torch.Tensor): Input tensor representing the coefficients.
        v (torch.Tensor): Input tensor representing the evaluation points.
        x (torch.Tensor): Input tensor representing the evaluation points.
        L (int): Number of evaluation points.

    Returns:
        torch.Tensor: Resulting tensor after applying the transpose of the Vandermonde matrix.
    """
    ...
    lvec = torch.arange(L).to(x)
    u, v, x, lvec = _broadcast_dims(u, v, x, lvec)
    u = _c2r(u)
    v = _c2r(v)
    x = _c2r(x)
    lvec = _c2r(lvec)

    r = vandermonde_mult(u, v, x, lvec, backend="GPU")
    return _r2c(r)


# Fallback versions
def cauchy_naive(v, z, w):
    """Computes the Cauchy kernel naively.
...
def log_vandermonde_naive(v, x, L, conj=True):
    """Computes the Vandermonde matrix naively.

    Args:
        v (torch.Tensor): Input tensor representing the coefficients.
        x (torch.Tensor): Input tensor representing the evaluation points.
        L (int): Number of evaluation points.
        conj (bool): Whether to use conjugate of v.
...
def log_vandermonde_transpose_naive(u, v, x, L):
    """Computes the transpose of the Vandermonde matrix naively.
    ...
    """
    vandermonde_matrix = torch.exp(x.unsqueeze(-1) * torch.arange(L).to(x))  # (... N L)
    return contract("... l, ... n, ... n l -> ... n", u.to(x), v.to(x), vandermonde_matrix)  # (... L)


""" Simple nn.Module components """


def Activation(activation=None, dim=-1):  # noqa: PLR0911
    """Returns an activation nn.Module given its name.

    Args:
        activation: Name of the activation function.
        dim: Dimension to apply the activation on (for GLU).
...
def LinearActivation(
    ...
    **kwargs,
    ...
    Args:
        ...
        transposed: Whether the layer is transposed.
        activation: Activation function to apply.
        activate: Whether to apply activation as part of this module.
        **kwargs: Additional keyword arguments for the linear layer.

    Returns:
        nn.Module: Linear layer with optional activation.
    """
...
class DropoutNd(nn.Module):
    """N-dimensional Dropout layer."""

    def __init__(self, p: float = 0.5, tie=True, transposed=True):
        """Initializes the DropoutNd layer.

        Args:
            p: dropout probability
            tie: tie dropout mask across sequence lengths (Dropout1d/2d/3d).
            transposed: whether the input tensor is transposed or not
        """
        super().__init__()
        if p < 0 or p >= 1:
        ...
        self.binomial = torch.distributions.binomial.Binomial(probs=1 - self.p)  # type: ignore

    def forward(self, X):
        """Forward pass of the DropoutNd layer.
...
def power(L, A, v=None):
    ...
    Args:
        L: Power to raise A to.
        A: (..., N, N)
        v: (..., N, L) or None

    Returns:
        (..., N, N), (..., N) if v is not None else (..., N, N)
    """
    ...
    # Note that an alternative divide and conquer to compute the
    # reduction is possible and can be embedded into the above loop
...
def transition(measure, N, **measure_args):
    ...
    Args:
        ...
        N: size of the recurrent A matrix (also known as `d_state` elsewhere).
        **measure_args: additional arguments for specific measures.
...
def rank_correction(measure, N, rank=1, dtype=torch.float):
    ...
    Args:
        measure: the type of measure
        N: size of the recurrent A matrix (also known as `d_state` elsewhere).
        rank: the rank of the low-rank matrix
        dtype: the data type of the matrix

    Returns:
        P: (rank, N) torch tensor
    """
    if measure == "legs":
        assert rank >= 1
...
def nplr(measure, N, rank=1, dtype=torch.float, diagonalize_precision=True, B_clip=2.0):
    ...
    Returns:
         w, p, q, V, B such that (w - p q^*, B) is unitarily equivalent to the original
            HiPPO A, B by the matrix V i.e. A = V[w - p q^*]V^*, B = V B
    """
    ...
    if (
        ...
        logging.warning("HiPPO matrix not skew symmetric by %d", err)
    ...
    # We use a huge hack here: Assume only one pair is 0, and that it is the first row/column
    # of A (only happens in Fourier case)
    V = V_sorted[:, : N // 2]
    ...
    if (err := torch.sum((2 * _AP.real - AP) ** 2) / N) > 1e-5:
        logging.warning("Warning: Diagonalization of A matrix not numerically precise - error %d", err)
...
def dplr(  # noqa: C901 PLR0912 PLR0915
    init="hippo",
    N=64,
    rank=1,
    H=1,
    ...
    Args:
        ...
        N: (int) Size of recurrent A matrix (also known as `d_state` elsewhere).
        rank: (int) Rank of low-rank correction.
        H: (int) Number of heads (independent copies of the same A, B, P).
        ...
        B_random: (bool) Deprecated, use B_init='random' instead.
    ...
    Returns:
        A: (H, N) torch tensor representing the diagonal of A matrix
        P: (rank, H, N) torch tensor
        B: (H, N) torch tensor
        V: (H, N, N) torch tensor representing the unitary matrix for diagonalization
    """
    assert dtype in (torch.float, torch.double)
    ...
    real_part = torch.rand(H, N // 2) if real_random else 0.5 * torch.ones(H, N // 2)
    real_part = real_scale * real_part

    # Construct imaginary part of diagonal A (must be non-negative)
    imag_part = N // 2 * torch.rand(H, N // 2) if imag_random else repeat(torch.arange(N // 2), "n -> h n", h=H)
    ...
    if init in ["random", "rand"]:
        ...
        real_part = (
            ...
            else 1 + repeat(torch.arange(N // 2), "n -> h n", h=H)
        )
    ...
    assert torch.all(A.real < 1e-4)  # Allow some tolerance for numerical precision on real part
    assert torch.all(A.imag <= 0.0)
...
def ssm(init, N, R, H, **ssm_args):
    ...
    Args:
        ...
        **ssm_args: additional arguments for specific SSM initializations

    Returns:
        A: (H, N) torch tensor representing the diagonal of A matrix
        P: (R, H, N) torch tensor
        B: (H, N) torch tensor
        V: (H, N, N) torch tensor representing the unitary matrix for diagonalization
    """
...
def combination(inits, N, R, S, **ssm_args):
    """Create multiple independent SSM copies with different initializations.

    Args:
        inits: (str | list[str]) Initialization methods
        N: state size
        R: rank (for DPLR parameterization)
        S: total number of independent SSM copies
        **ssm_args: additional arguments for specific SSM initializations

    Returns:
        A: (S, N) torch tensor representing the diagonal of A matrix
        P: (R, S, N) torch tensor
        B: (S, N) torch tensor
        V: (S, N, N) torch tensor representing the unitary matrix for diagonalization
    """
    if isinstance(inits, str):
        inits = combinations.get(inits, [inits])
...
def inv_transform(param, transform="none"):
    ...
    Args:
        param: (torch.Tensor) Parameter tensor to transform.
        transform: (str) Type of transform to apply.

    Returns:
        torch.Tensor: Transformed parameter tensor.
    """
...
class Kernel(nn.Module):
        ...
        **kwargs,
        ...
        Args:
            ...
            channels (C): Extra dimension in the returned output (see .forward()).
                - One interpretation is that it expands the input dimension giving
                it C separate "heads" per feature.
                That is convolving by this kernel maps shape (B L D) -> (B L C D)
                - This is also used to implement a particular form of bidirectionality
                in an efficient way.
            ...
            lr: Optional dictionary specifying special hyperparameters for .register().
                ...
                A custom optimizer hook is needed to configure the optimizer to set the
            ...
            **kwargs: Additional keyword arguments to customize the kernel.
        """
        super().__init__()
        assert d_model > 0
    ...
    def register(self, name, tensor, lr=None, wd=0.0):
        ...
        Args:
            ...
            lr: Learning rate for the tensor.
            wd: Weight decay for the tensor.
        """
        if lr == 0.0:
    ...
    def step(self, x, state, **kwargs):
        ...
        Args:
            x: Input at the current timestep.
...
class SSMKernel(Kernel):
    ...
    Options:
    d_state (N): State size (dimensionality of parameters A, B, C). Generally shouldn't need to be
                 adjusted and doens't affect speed much for most kernels (e.g. S4, S4D).
    deterministic: Use a deterministic initialization for dt, A, B, C.
                   Useful for debugging as well as constructing a simple exponential decay kernel
                   (e.g. used in S4ND image->video inflation).

    dt_min, dt_max: min and max values for the step size dt
    dt_tie: Keep dt tied across the N dimensions of the state. Although this theoretically makes more sense,
            models such as S5 and Mega have found slightly improvements by setting it to False.
    ...
    init: Options for initialization of (A, B). For DPLR mode, recommendations are "legs", "fout",
          "hippo" (combination of both). For Diag mode, recommendations are "diag-inv", "diag-lin",
          "diag-legs", and "diag" (combination of diag-inv and diag-lin).
    init_args: Extra arguments passed into initialization function (see dplr.py for options).
    """

    def init_dt(self):
        """Initialize the dt parameter.

        Returns:
            inv_dt: (H, 1) or (H, N/2) torch tensor
        """
    ...
    def init_ssm_real(self):
        """Initialize real-valued (A, B, C) parameters for SSM.

        Returns:
             (dense, real) (A, B, C) parameters for init options.
        """
        # Generate A, B
    ...
    def init_ssm_dplr(self):
        """Initialize DPLR (A, P, B, C) parameters for init options.

        Returns:
            DPLR (A, P, B, C) parameters for init options.
        """
        ...
        assert self.n_ssm % P.size(-2) == 0
        assert self.n_ssm % A.size(-2) == 0
    ...
    def __init__(
        ...
        **init_args,
        ...
        """Initialize SSM kernel with common options.

        Args:
            d_model (H): Model dimension, or number of independent convolution kernels created.
            channels (C): Extra dimension in the returned output (see .forward()).
            l_max (L): Maximum kernel length (optional).
            lr: Optional dictionary specifying special hyperparameters for .register().
            wd: Same as lr, but for weight decay.
            verbose: Whether to print verbose logging information.
            d_state (N): State size (dimensionality of parameters A, B, C
            deterministic: Use a deterministic initialization for dt, A, B, C.
            dt_min: dt_max: min and max values for the step size dt
            dt_max: max value for the step size dt
            dt_tie: Keep dt tied across the N dimensions of the state.
            dt_transform: Transform function for parameterization of dt (default 'softplus', used to be 'exp')
            rank: Rank of low-rank correction for DPLR mode.
            n_ssm: Number of independent trainable (A, B) SSMs.
            measure: (deprecated) Initialization method for (A, B). Use 'init' instead.
            init: Initialization method for (A, B).
            **init_args: Extra arguments passed into initialization function.
        """
    ...
    def d_state(self):
        ...
        Returns:
            int: The dimensionality of the state.
        """
        return self.H * self.N

    @property
    def state_to_tensor(self):
        """Convert state to tensor form.

        Returns:
            function: A function that converts state to tensor form.
        """
...
class SSMKernelDiag(SSMKernel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize SSM kernel with diagonal state matrix.
        ...
        Args:
            ...
            imag_transform: ['none' | 'exp' | 'relu' | 'sigmoid' | 'softplus']
            bandlimit: Mask high frequencies of the kernel
                (indices corresponding to diagonal elements with large imaginary part). Introduced in S4ND paper.
            backend: ['keops' | 'naive'] Options for Vandermonde/Cauchy kernel (in order of efficiency).
            is_real : Real-valued SSM; can be interpreted as EMA.
            **kwargs: Additional arguments for SSMKernel parent class.
        """
    ...
    def register_params(self, A, B, C, inv_dt, P):
        ...
        Args:
            ...
            B: (S, N)
            C: (C, H, N)
            inv_dt: (H) timescale per feature
            P: (H, H) transition matrix
        """
        assert self.backend in ["keops", "naive"]

        if self.dt_fast:
            inv_dt = torch.asinh(inv_dt)

        # Rank of low-rank correction
        assert inv_dt.size(0) == self.H
    ...
    def _get_params(self, rate=1.0):
        ...
        Returns:
            dt: (H, N) step size
            A: (H, N) diagonal state matrix
            B: (1, H, N) input matrix
            C: (C, H, N) output matrix
        """
        # (S N) where S=n_ssm
        if self.is_real:
        ...
        inv_dt = torch.sinh(self.inv_dt) if self.dt_fast else self.inv_dt
    ...
    def forward(self, L, state=None, rate=1.0):
        ...
        log_vandermonde = log_vandermonde_keops if self.backend == "keops" else log_vandermonde_naive
        ...
        K_state = K[:-1, :, :, :] if state is not None else None  # (B C H L)
        K = K[-1, :, :, :]  # (C H L)
    ...
    def default_state(self, *batch_shape):
        ...
        Args:
            *batch_shape: Additional batch shape.

        Returns:
            (B, H, N) tensor of zeros.
        """
        C = _r2c(self.C)
        return torch.zeros(*batch_shape, self.H, self.N, dtype=C.dtype, device=C.device)

    def step(self, u, state):
        """Step the model for one timestep with input u and recurrent state.

        Args:
            u: (B, H) input at the current timestep.
            state: (B, H, N) current recurrent state.

        Returns:
            y: (B, C, H) output at the current timestep.
            next_state: (B, H, N) next recurrent state.
        """
    ...
    def forward_state(self, u, state):
        ...
        Returns:
            (B, H, N) final state after passing through the sequence.
        """
        self._setup_step()
        AL = self.dA ** u.size(-1)
...
class SSMKernelDPLR(SSMKernelDiag):
    ...
    def _setup_C(self, L):
        ...
        if self.l_kernel.item() == 0:
            ...
            double_length = False
        elif self.l_kernel.item() < L:  # 2*int(self.l_kernel) == L:
            if self.verbose:
    ...
    def _omega(self, L, dtype, device, cache=True):
        ...
        Args:
            L: Length of the FFT nodes.
            dtype: Data type of the returned tensors.
            device: Device of the returned tensors.
            cache: Whether to cache the computed tensors.

        Returns:
            omega: (L//2 + 1) FFT nodes.
            z: (L//2 + 1) bilinear transformed nodes.
        """
    ...
    def forward(self, state=None, rate=1.0, L=None):  # PLR0915
        """See Kernel.forward() for argument documentation.
        ...
        # The idea is that this kernel's length (in continuous units) is self.l_kernel,
        # while we are asked to provide a kernel of length L at (relative) frequency rate
        if L is None:
        ...
        # Incorporate dt into B
        v = v * dt
        # Dispatch which Cauchy kernel to use
        cauchy_mult = cauchy_keops if self.backend in "keops" else cauchy_naive
    ...
    def _setup_linear(self):
        ...
        dt, A, B, _C, P, Q = self._get_params()
        ...
        try:
            ...
            R = torch.tensor(
                np.linalg.solve(R.to(Q_D).contiguous().detach().cpu(), Q_D.contiguous().detach().cpu())
            ).to(Q_D)
    ...
    def _step_state_linear(self, u=None, state=None):
        ...
        This takes advantage of the DPLR form and bilinear discretization.
        Unfortunately, as currently implemented it's about 2x slower because it calls several sequential
        operations. Perhaps a fused CUDA kernel implementation would be much faster.
        ...
        Returns:
            new_state: (H, N/2) or (H, N) New state
        """
        ...
        return D * (new_state - contract_fn(P, R, new_state))

    def _setup_state(self):
        """Construct dA and dB for discretized state equation.

        Returns:
            dA: (H, N, N) Discretized state matrix
            dB: (H, N) Discretized input matrix
        """
    ...
    def _step_state(self, u, state):
        ...
        Returns:
            new_state: (H, N/2) or (H, N) New state
        """
        return torch.einsum(self.state_contraction, self.dA, state) + torch.einsum(self.input_contraction, self.dB, u)

    def _setup_step(self, mode="dense"):
        """Set up dA, dB, dC discretized parameters for stepping.

        Args:
            mode: 'dense' | 'linear' | 'diagonal' Step mode to use.
        """
        ...
        if mode == "linear":
            ...
            if self.verbose:
                logging.warning("Diagonalization error: %d", torch.dist(V @ torch.diag_embed(L) @ V_inv, self.dA))
    ...
    def default_state(self, *batch_shape):
        ...
        return torch.zeros(*batch_shape, H, N, dtype=C.dtype, device=C.device)

    def step(self, u, state):
        """Must have called self._setup_step() and created state with self.default_state() before calling this.

        Args:
            u: (B, H) Input at current timestep.
            state: (B, H, N) Current recurrent state.

        Returns:
            y: (B, C, H) Output at current timestep.
            next_state: (B, H, N) Next recurrent state.
        """
        new_state = self._step_state_linear(u, state) if self._step_mode == "linear" else self._step_state(u, state)
    ...
    def forward_state(self, *args, **kwargs):
        ...
        Args:
            *args: see SSMKernel.forward_state()
            **kwargs: see SSMKernel.forward_state()

        Returns:
            see SSMKernel.forward_state()
        """
